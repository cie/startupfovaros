...
Meteor.methods({
  comment: function(postId, parentCommentId, text){
    ...
    if (!user || !canComment(user))
      throw new Meteor.Error(i18n('You need to login or be invited to post new comments.'));
    
    // check that user waits more than 15 seconds between comments
    if(!this.isSimulation && (timeSinceLastComment < commentInterval))
      throw new Meteor.Error(704, i18n('Please wait ')+(commentInterval-timeSinceLastComment)+i18n.t(' seconds before commenting again'));

    // Don't allow empty comments
    if (!cleanText)
      throw new Meteor.Error(704,i18n('Your comment is empty.'));

...
var filters = {
  ...
  isLoggedIn: function(pause) {
    if (!(Meteor.loggingIn() || Meteor.user())) {
      throwError(i18n('Please Sign In First.'))
      this.render('signin');
      pause(); 
    }
  },
  ...
  isAdmin: function(pause) {
    ...
    if(!isAdmin()){
      throwError(i18n("Sorry, you  have to be an admin to view this page."))
  ...
  canPost: function (pause) {
    if(!this.ready()) return;
    if(!canPost()){
      throwError(i18n("Sorry, you don't have permissions to add new items."))
      this.render('no_rights');
      pause();      
    }
  },
  ...
  canEditPost: function(pause) {
    ...
    if(!currentUserCanEdit(post)){
      throwError(i18n("Sorry, you cannot edit this post."))
      this.render('no_rights');
      pause();
    }
  },
  ...
  canEditComment: function(pause) {
    ...
    if(!currentUserCanEdit(comment)){
      throwError(i18n("Sorry, you cannot edit this comment."))
      this.render('no_rights');
      pause();
    }
  },
